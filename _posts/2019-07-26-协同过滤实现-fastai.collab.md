...
以下介绍基于fastai课程的第五课中对协同过滤的相关介绍以及fastai源码
视频课程留下印象最深的一句话是“it's not awesome”，所以必须要清楚一个函数背后做了什么，参数的意义是什么，以及为什么要这样设置，这样会对深度学习有更深刻的理解。

## 协同过滤（Collaborative filtering）简介
协同过滤算法可以用来预测某个用户有多大可能性会喜欢某个事物（Collaborative filtering is when you're tasked to predict how much a user is going to like a certain item），属于推荐系统问题。
